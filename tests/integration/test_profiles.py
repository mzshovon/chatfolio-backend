...
async def test_profile_is_isolated_per_user() -> None:
    ...
    await client.post(
        "/api/v1/auth/register", json={"email": other_email, "password": "supersecret123"}
    )
    other_login = await client.post(
        "/api/v1/auth/login", json={"email": other_email, "password": "supersecret123"}
    )
    other_headers = {"Authorization": f"Bearer {other_login.json()['access_token']}"}

    await client.patch("/api/v1/profiles/me", headers=headers, json={"full_name": "Owner One"})
    other_response = await client.get("/api/v1/profiles/me", headers=other_headers)
...
async def test_experience_crud() -> None:
    ...
    list_response = await client.get("/api/v1/profiles/me/experience", headers=headers)
    ...
    empty_list_response = await client.get("/api/v1/profiles/me/experience", headers=headers)
