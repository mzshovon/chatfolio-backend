...
async def test_get_status_and_retry_after_failure() -> None:
    ...
    status_response = await client.get(f"/api/v1/cv/{cv_id}/status", headers=headers)
    assert status_response.status_code == 200
    assert status_response.json()["status"] == "pending"

    retry_before_failure = await client.post(f"/api/v1/cv/{cv_id}/retry", headers=headers)
    assert retry_before_failure.status_code == 422

    await _mark_failed(cv_id)

    failed_status_response = await client.get(f"/api/v1/cv/{cv_id}/status", headers=headers)
    assert failed_status_response.json()["status"] == "failed"
    assert failed_status_response.json()["error_message"] == "Could not extract text."

    retry_response = await client.post(f"/api/v1/cv/{cv_id}/retry", headers=headers)
...
async def test_cannot_access_another_users_cv() -> None:
    ...
    response = await client.get(f"/api/v1/cv/{cv_id}/status", headers=other_headers)
