...
async def test_forgot_password_then_reset_allows_login_with_new_password() -> None:
    ...
    reset_response = await client.post(
        "/v1/auth/reset-password", json={"token": token, "new_password": "brandnewpass123"}
    )
    assert reset_response.status_code == 204

    old_login = await client.post(
        "/v1/auth/login", json={"email": email, "password": "supersecret123"}
    )
    assert old_login.status_code == 401

    new_login = await client.post(
        "/v1/auth/login", json={"email": email, "password": "brandnewpass123"}
    )
...
async def test_forgot_password_for_unknown_email_returns_204_without_sending() -> None:
    ...
    response = await client.post(
        "/v1/auth/forgot-password", json={"email": "never-registered@example.com"}
    )
    assert response.status_code == 204
...
async def test_reset_password_token_is_single_use() -> None:
    ...
    first = await client.post(
        "/v1/auth/reset-password", json={"token": token, "new_password": "firstnewpass123"}
    )
    assert first.status_code == 204

    second = await client.post(
        "/v1/auth/reset-password", json={"token": token, "new_password": "secondnewpass123"}
    )
