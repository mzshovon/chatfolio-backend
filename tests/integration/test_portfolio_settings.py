...
async def _approve_both_sections(
    ...
    for section in sections:
        await client.post(f"/v1/sections/{section['id']}/approve", headers=headers)
...
async def test_get_settings_auto_creates_with_generated_slug() -> None:
    ...
    response = await client.get("/v1/portfolio-settings", headers=headers)
...
async def test_publish_fails_without_name_or_approved_sections() -> None:
    ...
    response = await client.post("/v1/portfolio-settings/publish", headers=headers)
...
async def test_publish_succeeds_once_requirements_met(set_fake_llm: Callable[[str], None]) -> None:
    ...
    await client.patch("/v1/profiles/me", headers=headers, json={"full_name": "Ada Lovelace"})
...
async def test_unpublish_hides_the_page(set_fake_llm: Callable[[str], None]) -> None:
    ...
    await client.patch("/v1/profiles/me", headers=headers, json={"full_name": "Ada Lovelace"})
    await _approve_both_sections(client, headers, set_fake_llm)
    await client.post("/v1/portfolio-settings/publish", headers=headers)

    response = await client.post("/v1/portfolio-settings/unpublish", headers=headers)
    assert response.status_code == 200
...
async def test_slug_update_tracks_previous_slug_and_enforces_uniqueness() -> None:
    ...
    original = (await client_a.get("/v1/portfolio-settings", headers=headers_a)).json()["slug"]

    rename_response = await client_a.patch(
        "/v1/portfolio-settings", headers=headers_a, json={"slug": "ada-lovelace-dev"}
    )
    ...
    conflict_response = await client_b.patch(
        "/v1/portfolio-settings", headers=headers_b, json={"slug": "ada-lovelace-dev"}
    )
